...
const mockService = (content: string) =>
  ({
    readFile: () => Promise.resolve(Buffer.from(content)),
  }) as unknown as DocumentsService;

const failingService = (message: string) =>
  ({
    readFile: () => Promise.reject(new Error(message)),
  }) as unknown as DocumentsService;
...
describe('readDocument', () => {
  ...
  describe('JSON files', () => {
    it('returns parsed object', async () => {
      ...
      expect(result.success).toBe(true);
      const config = result.content as { database: { pool: { max: number } } };
  ...
  describe('error handling', () => {
    it('returns failure when file not found', async () => {
      const result = await readDocument(
        failingService('File not found: "missing.txt".'),
        'missing.txt',
      );
    ...
    it('returns failure when size cap exceeded', async () => {
      const result = await readDocument(
        failingService('exceeds the 100 KB read limit'),
        'big.txt',
      );
      expect(result.success).toBe(false);
